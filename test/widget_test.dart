...
void main() {
  // The welcome card needs more vertical space than the default 800x600
  // test viewport.
  ...
  testWidgets('splash shows brand identity then routes to welcome',
    ...
    expect(find.byKey(const Key('welcome-email-button')), findsNothing);
    expect(find.byKey(const Key('welcome-register-button')), findsNothing);
    expect(find.byKey(const Key('welcome-forgot-button')), findsNothing);
  });

  testWidgets('splash routes authenticated users through auth loading to home',
      (tester) async {
    await useTabletViewport(tester);
